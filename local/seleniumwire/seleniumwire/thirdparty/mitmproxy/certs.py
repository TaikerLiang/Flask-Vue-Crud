...
def dummy_cert(privkey, cacert, commonname, sans, organization):
    ...
    cert.add_extensions([OpenSSL.crypto.X509Extension(b"extendedKeyUsage", False, b"serverAuth,clientAuth")])
...
class CertStore:
    ...
    def from_store(cls, path, basename, key_size, passphrase: typing.Optional[bytes] = None):
        ...
        if not os.path.exists(ca_path):
            ...
            key = OpenSSL.crypto.load_privatekey(OpenSSL.crypto.FILETYPE_PEM, raw, passphrase)
    ...
    def add_cert_file(self, spec: str, path: str, passphrase: typing.Optional[bytes] = None) -> None:
        with open(path, "rb") as f:
            raw = f.read()
        cert = Cert(OpenSSL.crypto.load_certificate(OpenSSL.crypto.FILETYPE_PEM, raw))
        try:
            privatekey = OpenSSL.crypto.load_privatekey(OpenSSL.crypto.FILETYPE_PEM, raw, passphrase)
        except Exception:
            privatekey = self.default_privatekey
        self.add_cert(CertStoreEntry(cert, privatekey, path), spec.encode("idna"))
    ...
    def get_cert(
        self, commonname: typing.Optional[bytes], sans: typing.List[bytes], organization: typing.Optional[bytes] = None
        ...
        if name:
            ...
            entry = CertStoreEntry(
                cert=dummy_cert(self.default_privatekey, self.default_ca, commonname, sans, organization),
                privatekey=self.default_privatekey,
...
class _GeneralName(univ.Choice):
    ...
    componentType = namedtype.NamedTypes(
        ...
        namedtype.NamedType(
            'iPAddress', univ.OctetString().subtype(implicitTag=tag.Tag(tag.tagClassContext, tag.tagFormatSimple, 7))
        ),
    )
...
class Cert(serializable.Serializable):
    ...
    def keyinfo(self):
        ...
        return (types.get(pk.type(), "UNKNOWN"), pk.bits())
