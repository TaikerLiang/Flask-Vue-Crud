...
class Reply:
    ...
    def take(self):
        ...
        """
        if self.state != "start":
            raise exceptions.ControlException("Reply is {}, but expected it to be start.".format(self.state))
        self._state = "taken"
    ...
    def send(self, msg, force=False):
        if self.state not in {"start", "taken"}:
            raise exceptions.ControlException("Reply is {}, but expected it to be start or taken.".format(self.state))
