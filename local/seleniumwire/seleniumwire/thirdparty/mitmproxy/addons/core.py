    ...
    def configure(self, updated):
        ...
        if "body_size_limit" in updated:
            try:
                ...
                raise exceptions.OptionsError("Invalid body size limit specification: %s" % opts.body_size_limit)
        if "mode" in updated:
            mode = opts.mode
    ...
    def flow_set(self, flows: typing.Sequence[flow.Flow], attr: str, value: str) -> None:
        ...
        if attr == "status_code":
            try:
                ...
                raise exceptions.CommandError("Status code is not an integer: %s" % val) from v
            ...
            if req:
                if attr == "method":
                    ...
                    try:
                        ...
                        raise exceptions.CommandError("URL %s is invalid: %s" % (repr(val), e)) from e
                else:
                    self.rupdate = False
