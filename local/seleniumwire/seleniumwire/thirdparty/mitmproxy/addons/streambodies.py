...
class StreamBodies:
    ...
    def run(self, f, is_request):
        if self.max_size:
            ...
            try:
                expected_size = http1.expected_http_body_size(f.request, f.response if not is_request else None)
            ...
            if expected_size and not r.raw_content and not (0 <= expected_size <= self.max_size):
                ...
                ctx.log.info(
                    "Streaming {} {}".format("response from" if not is_request else "request to", f.request.host)
                )
