...
def MIB_TCP6TABLE_OWNER_PID(size):
    class _MIB_TCP6TABLE_OWNER_PID(ctypes.Structure):
        _fields_ = [('dwNumEntries', ctypes.wintypes.DWORD), ('table', MIB_TCP6ROW_OWNER_PID * size)]
...
def MIB_TCPTABLE_OWNER_PID(size):
    class _MIB_TCPTABLE_OWNER_PID(ctypes.Structure):
        _fields_ = [('dwNumEntries', ctypes.wintypes.DWORD), ('table', MIB_TCPROW_OWNER_PID * size)]
...
class TransparentProxy:
    ...
    def __init__(
        ...
        self.filter = (
            filter or f"tcp.DstPort != {proxy_port} and tcp.DstPort != {REDIRECT_API_PORT} and tcp.DstPort < 49152"
        )
        ...
        if forward:
            self.forward = Redirect(self.redirect_request, self.filter, pydivert.Layer.NETWORK_FORWARD)
        if local:
