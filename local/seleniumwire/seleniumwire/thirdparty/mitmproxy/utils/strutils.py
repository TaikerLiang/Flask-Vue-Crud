...
_control_char_trans = {x: ord(".") for x in range(32)}  # x + 0x2400 for unicode control group pictures
...
def bytes_to_escaped_str(data, keep_spacing=False, escape_single_quotes=False):
    ...
    if keep_spacing:
        ret = re.sub(
            r"(?<!\\)(\\\\)*\\([nrt])", lambda m: (m.group(1) or "") + dict(n="\n", r="\r", t="\t")[m.group(2)], ret
        )
    return ret
...
def hexdump(s):
    ...
    for i in range(0, len(s), 16):
        ...
        part_repr = always_str(
            escape_control_characters(part.decode("ascii", "replace").replace(u"\ufffd", u"."), False)
        )
