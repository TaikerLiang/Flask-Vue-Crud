...
class _LayerCodeCompletion:
    ...
    def __init__(self, **mixin_args):  # pragma: no cover
        ...
        self.ctx = None
        """@type: seleniumwire.thirdparty.mitmproxy.server.protocol.Layer"""
...
class ServerConnectionMixin:
    ...
    def __check_self_connect(self):
        ...
        if address:
            ...
            self_connect = address[1] == self.config.options.listen_port and address[0] in forbidden_hosts
            if self_connect:
                raise exceptions.ProtocolException(
                    "Invalid server address: {}\r\n" "The mitmproxy shall not connect to itself.".format(repr(address))
                )
    ...
    def __make_server_conn(self, server_address):
        if self.config.options.spoof_source_address and self.config.options.upstream_bind_address == '':
            return connections.ServerConnection(server_address, (self.ctx.client_conn.address[0], 0), True)
        else:
            return connections.ServerConnection(
                server_address, (self.config.options.upstream_bind_address, 0), self.config.options.spoof_source_address
            )
    ...
    def connect(self):
        ...
        try:
            ...
            raise exceptions.ProtocolException(
                "Server connection to {} failed: {}".format(repr(self.server_conn.address), str(e))
            )
