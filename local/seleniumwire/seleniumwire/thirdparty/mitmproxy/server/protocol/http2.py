...
from seleniumwire.thirdparty.mitmproxy import connections, exceptions, http  # noqa
...
from seleniumwire.thirdparty.mitmproxy.net.http import headers, http2, url
from seleniumwire.thirdparty.mitmproxy.server.protocol import http as httpbase, base
...
class Http2Layer(base.Layer):
    ...
    def _handle_event(self, event, source_conn, other_conn, is_server):
        self.log("HTTP2 Event from {}".format("server" if is_server else "client"), "debug", [repr(event)])
    ...
    def _handle_request_received(self, eid, event):
        headers = seleniumwire.thirdparty.mitmproxy.net.http.Headers([[k, v] for k, v in event.headers])
    ...
    def _handle_data_received(self, eid, event, source_conn):
        ...
        if bsl and self.streams[eid].queued_data_length > bsl:
            ...
            self.connections[source_conn].safe_reset_stream(event.stream_id, h2.errors.ErrorCodes.REFUSED_STREAM)
        ...
        self.connections[source_conn].safe_acknowledge_received_data(event.flow_controlled_length, event.stream_id)
        return True
    ...
    def _handle_trailers(self, eid, event, is_server, other_conn):
        trailers = seleniumwire.thirdparty.mitmproxy.net.http.Headers([[k, v] for k, v in event.headers])
    ...
    def _handle_connection_terminated(self, event, is_server):
        self.log(
            "HTTP/2 connection terminated by {}: error code: {}, last stream id: {}, additional data: {}".format(
                "server" if is_server else "client", event.error_code, event.last_stream_id, event.additional_data
            ),
            "info",
        )
        ...
        if event.error_code != h2.errors.ErrorCodes.NO_ERROR:
            ...
            self.connections[self.client_conn].close_connection(
                error_code=event.error_code, last_stream_id=event.last_stream_id, additional_data=event.additional_data
            )
            self.client_conn.send(self.connections[self.client_conn].data_to_send())
...
class Http2SingleStreamLayer(httpbase._HttpTransmissionLayer, basethread.BaseThread):
    class Message:
        def __init__(self, headers=None):
            ...
            ] = headers  # headers are the first thing to be received on a new stream
            self.data_queue: queue.Queue[bytes] = queue.Queue()  # contains raw contents of DATA frames
            self.queued_data_length = (
                0  # used to enforce seleniumwire.thirdparty.mitmproxy's config.options.body_size_limit
            )
            self.trailers: Optional[
                seleniumwire.thirdparty.mitmproxy.net.http.Headers
            ] = None  # trailers are received after stream_ended is set
    ...
    def __init__(
        self, ctx, h2_connection, stream_id: int, request_headers: seleniumwire.thirdparty.mitmproxy.net.http.Headers
    ) -> None:
        super().__init__(ctx, name="Http2SingleStreamLayer-{}".format(stream_id))
    ...
    def send_request_headers(self, request):
        ...
        if self.handled_priority_event:
            ...
            if not self.config.options.http2_priority:
                self.log(
                    "HTTP/2 PRIORITY information in HEADERS frame suppressed. Use --http2-priority to enable forwarding.",
                    "debug",
                )
            else:
    ...
    def send_response_headers(self, response):
        ...
        with self.connections[self.client_conn].lock:
            self.connections[self.client_conn].safe_send_headers(self.raise_zombie, self.client_stream_id, headers)
    ...
    def _send_trailers(self, conn, trailers):
        ...
        with self.connections[conn].lock:
            self.connections[conn].safe_send_headers(
                self.raise_zombie, self.client_stream_id, trailers, end_stream=True
            )
