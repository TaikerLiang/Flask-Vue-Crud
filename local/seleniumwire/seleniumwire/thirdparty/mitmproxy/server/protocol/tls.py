...
class TlsLayer(base.Layer):
    ...
    def __call__(self):
        ...
        client_tls_requires_server_connection = (
            self._server_tls
            and self.config.options.upstream_cert
            and (
                self.config.options.add_upstream_certs_to_client_chain
                or self._client_tls
                and (self._client_hello.alpn_protocols or not self._client_hello.sni)
            )
        )
        establish_server_tls_now = (
            self.server_conn.connected() and self._server_tls
    ...
    def _establish_tls_with_client(self):
        ...
        try:
            ...
            raise exceptions.ClientHandshakeException(
                "Cannot establish TLS with client (sni: {sni}): {e}".format(sni=sni_str, e=repr(e)),
    ...
    def _establish_tls_with_server(self):
        ...
        try:
            ...
            self.server_conn.establish_tls(sni=self.server_sni, alpn_protos=alpn, **args)
            ...
            raise exceptions.TlsProtocolException(
                "Cannot establish TLS with {host}:{port} (sni: {sni}): {e}".format(
                    host=self.server_conn.address[0], port=self.server_conn.address[1], sni=self.server_sni, e=repr(e)
                )
            )
    ...
    def _find_cert(self):
        ...
        use_upstream_cert = self.server_conn and self.server_conn.tls_established and self.config.options.upstream_cert
