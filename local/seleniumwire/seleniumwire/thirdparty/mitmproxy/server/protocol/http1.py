...
class Http1Layer(httpbase._HttpTransmissionLayer):
    ...
    def read_request_body(self, request):
        ...
        return http1.read_body(
            self.client_conn.rfile, expected_size, human.parse_size(self.config.options.body_size_limit)
        )
    ...
    def read_response_body(self, request, response):
        ...
        return http1.read_body(
            self.server_conn.rfile, expected_size, human.parse_size(self.config.options.body_size_limit)
        )
    ...
    def check_close_connection(self, flow):
        request_close = http1.connection_close(flow.request.http_version, flow.request.headers)
        response_close = http1.connection_close(flow.response.http_version, flow.response.headers)
