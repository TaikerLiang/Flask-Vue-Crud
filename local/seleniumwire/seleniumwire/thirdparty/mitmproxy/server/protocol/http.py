...
class _HttpTransmissionLayer(base.Layer):
    ...
    def read_response(self, request):
        ...
        response.data.content = b"".join(self.read_response_body(request, response))
    ...
    if request.first_line_format not in allowed_request_forms:
        ...
        if mode == HTTPMode.transparent:
            ...
            err_message = "Invalid HTTP request form (expected: %s, got: %s)" % (
                " or ".join(allowed_request_forms),
                request.first_line_format,
            )
...
class HttpLayer(base.Layer):
    ...
    def __call__(self):
        ...
        while True:
            flow = http.HTTPFlow(self.client_conn, self.server_conn, live=self, mode=self.mode.name)
    ...
    def handle_upstream_connect(self, f):
        ...
        if not f.response:
            self.establish_server_connection(f.request.host, f.request.port, f.request.scheme)
            self.send_request(f.request)
            f.response = self.read_response_headers()
            f.response.data.content = b"".join(self.read_response_body(f.request, f.response))
    ...
    def _process_flow(self, f):
        try:
            ...
            if request.first_line_format == "authority":
                ...
                f.request.data.content = b"".join(self.read_request_body(f.request))
        ...
        try:
            ...
            if f.response.status_code == 101:
                ...
                is_websocket = websockets.check_handshake(f.request.headers) and websockets.check_handshake(
                    f.response.headers
                )
                if is_websocket and not self.config.options.websocket:
                    self.log("Client requested WebSocket connection, but the protocol is disabled.", "info")
            ...
            if not f.response:
                ...
                raise exceptions.ProtocolException("Error in HTTP connection: %s" % repr(e))
        finally:
            if f:
                f.live = False
