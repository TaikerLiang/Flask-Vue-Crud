from seleniumwire.thirdparty.mitmproxy import exceptions, log
from seleniumwire.thirdparty.mitmproxy.net import tls
from seleniumwire.thirdparty.mitmproxy.server import modes, protocol
from seleniumwire.thirdparty.mitmproxy.server.protocol import http
...
class RootContext:
    ...
    def log(self, msg, level, subs=()):
        ...
        """
        full_msg = ["{}:{}: {}".format(self.client_conn.address[0], self.client_conn.address[1], msg)]
        for i in subs:
            full_msg.append("  -> " + i)
        full_msg = "\n".join(full_msg)
