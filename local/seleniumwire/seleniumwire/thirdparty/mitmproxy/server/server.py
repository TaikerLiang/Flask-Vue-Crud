...
from seleniumwire.thirdparty.mitmproxy import connections, exceptions, flow, http, log, platform
...
class ProxyServer(tcp.TCPServer):
    ...
    def __init__(self, config: config.ProxyConfig) -> None:
        ...
        try:
            ...
            if self.socket:
                self.socket.close()
            raise exceptions.ServerException("Error starting proxy server: " + repr(e)) from e
...
class ConnectionHandler:
    def __init__(self, client_conn, client_address, config, channel):
        ...
        self.client_conn = connections.ClientConnection(client_conn, client_address, None)
        """@type: seleniumwire.thirdparty.mitmproxy.server.connection.ClientConnection"""
        self.channel = channel
        """@type: seleniumwire.thirdparty.mitmproxy.controller.Channel"""
        ...
        if mode.startswith("upstream:"):
            if "socks" in mode:
                return modes.SocksUpstreamProxy(
                    root_ctx, self.config.upstream_server, self.config.options.upstream_auth
                )
            else:
                return modes.HttpUpstreamProxy(root_ctx, self.config.upstream_server.address)
        elif mode == "regular":
