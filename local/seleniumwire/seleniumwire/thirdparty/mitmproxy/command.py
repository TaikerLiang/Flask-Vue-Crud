...
class Command:
    ...
    def call(self, args: typing.Sequence[str]) -> typing.Any:
        ...
        if not typ.is_valid(self.manager, typ, ret):
            raise exceptions.CommandError(f"{self.name} returned unexpected data - expected {typ.display}")
        return ret
...
class CommandManager:
                ...
                try:
                    ...
                    if is_command:
                        try:
                            ...
                            self.master.log.warn("Could not load command %s: %s" % (o.command_name, e))
    ...
    def execute(self, cmdstr: str) -> typing.Any:
        ...
        command_name, *args = [
            unquote(part.value) for part in parts if part.type != seleniumwire.thirdparty.mitmproxy.types.Space
        ]
