...
class _FlowType(_BaseFlowType):
    ...
    def parse(self, manager: "CommandManager", t: type, s: str) -> flow.Flow:
        try:
            ...
            raise exceptions.TypeError(str(e)) from e
        if len(flows) != 1:
            raise exceptions.TypeError("Command requires one flow, specification matched %s." % len(flows))
        return flows[0]
...
class _DataType(_BaseType):
    ...
    def completion(self, manager: "CommandManager", t: type, s: str) -> typing.Sequence[str]:  # pragma: no cover
        raise exceptions.TypeError("data cannot be passed as argument")

    def parse(self, manager: "CommandManager", t: type, s: str) -> typing.Any:  # pragma: no cover
        raise exceptions.TypeError("data cannot be passed as argument")
