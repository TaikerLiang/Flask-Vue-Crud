    ...
    if expected_size is None:
        ...
        if limit is not None and expected_size > limit:
            raise exceptions.HttpException(
                "HTTP Body too large. " "Limit is {}, content length was advertised as {}".format(limit, expected_size)
            )
...
def expected_http_body_size(
    request: request.Request, response: typing.Optional[response.Response] = None, expect_continue_as_0: bool = True
):
    """
    Args:
    ...
    while True:
        ...
        if line[0] in b" \t":
            ...
            try:
                ...
                raise exceptions.HttpSyntaxException("Invalid header line: %s" % repr(line))
...
def _read_chunked(rfile, limit=sys.maxsize):
    ...
    while True:
        ...
        if line != b"\r\n" and line != b"\n":
            ...
            if total > limit:
                raise exceptions.HttpException(
                    "HTTP Body too large. Limit is {}, " "chunked content longer than {}".format(limit, total)
                )
