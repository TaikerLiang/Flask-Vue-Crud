...
def _assemble_request_line(request_data):
    ...
    if request_data.method.upper() == b"CONNECT":
        return b"%s %s %s" % (request_data.method, request_data.authority, request_data.http_version)
        ...
        return b"%s %s %s" % (request_data.method, request_data.path, request_data.http_version)
