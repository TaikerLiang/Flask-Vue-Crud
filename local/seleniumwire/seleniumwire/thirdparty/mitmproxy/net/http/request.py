...
class Request(message.Message):
    ...
    def make(
        ...
        if isinstance(headers, Headers):
            ...
            headers = Headers(
                (always_bytes(k, "utf-8", "surrogateescape"), always_bytes(v, "utf-8", "surrogateescape"))
                for k, v in headers.items()
            )
        elif isinstance(headers, Iterable):
            headers = Headers(headers)
        else:
            raise TypeError("Expected headers to be an iterable or dict, but is {}.".format(type(headers).__name__))
    ...
    def pretty_url(self) -> str:
        ...
        pretty_host, pretty_port = seleniumwire.thirdparty.mitmproxy.net.http.url.parse_authority(
    ...
    def constrain_encoding(self) -> None:
        ...
        if accept_encoding:
            self.headers["accept-encoding"] = ', '.join(
                e for e in {"gzip", "identity", "deflate", "br", "zstd"} if e in accept_encoding
            )
    ...
    def urlencoded_form(self):
        ...
        """
        return multidict.MultiDictView(self._get_urlencoded_form, self._set_urlencoded_form)
    ...
    def multipart_form(self):
        ...
        """
        return multidict.MultiDictView(self._get_multipart_form, self._set_multipart_form)
