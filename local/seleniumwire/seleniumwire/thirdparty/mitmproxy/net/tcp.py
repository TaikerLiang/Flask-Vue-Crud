    ...
    def safe_read(self, length):
        ...
        if length != -1 and len(result) != length:
            if not result:
                ...
                raise exceptions.TcpReadIncomplete("Expected %s bytes, got %s" % (length, len(result)))
        return result
...
def ssl_read_select(rlist, timeout):
    ...
    """
    return [conn for conn in rlist if isinstance(conn, SSL.Connection) and conn.pending() > 0] or select.select(
        rlist, (), (), timeout
    )[0]
...
class TCPClient(_Connection):
    ...
    def convert_to_tls(self, sni=None, alpn_protos=None, **sslctx_kwargs):
        context = tls.create_client_context(alpn_protos=alpn_protos, sni=sni, **sslctx_kwargs)
            ...
            try:
                ...
                if self.spoof_source_address:
                    try:
                        ...
                        if sock is not None:
                            sock.close()
                        raise exceptions.TcpException("Failed to spoof the source address: " + str(e))
                sock.connect(sa)
                return sock
    ...
    def connect(self):
        try:
            ...
            raise exceptions.TcpException('Error connecting to "%s": %s' % (self.address[0], err))
        self.connection = connection
...
class TCPServer:
    ...
    def handle_error(self, connection_, client_address, fp=sys.stderr):
        ...
        if traceback:
            ...
            print(u'-' * 40, file=fp)
            print(u"Error in processing of request from %s" % repr(client_address), file=fp)
            print(exc, file=fp)
            print(u'-' * 40, file=fp)
    ...
    def wait_for_silence(self, timeout=5):
        ...
        while 1:
            if time.time() - start >= timeout:
                raise exceptions.Timeout("%s service threads still alive" % self.handler_counter.count)
