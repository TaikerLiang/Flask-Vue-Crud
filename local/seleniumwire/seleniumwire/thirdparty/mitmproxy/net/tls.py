...
class MasterSecretLogger:
    ...
    def __call__(self, connection, where, ret):
        done_now = where == SSL.SSL_CB_HANDSHAKE_DONE and ret == 1
        ...
        done_previously_but_not_logged_yet = hasattr(connection, "_still_needs_masterkey")
...
log_master_secret = MasterSecretLogger.create_logfun(os.getenv("MITMPROXY_SSLKEYLOGFILE") or os.getenv("SSLKEYLOGFILE"))
...
def _create_ssl_context(
    ...
    verify_callback: typing.Optional[typing.Callable[[SSL.Connection, SSL.X509, int, int, bool], bool]] = None,
) -> SSL.Context:
    """
    ...
    if verify is not None:
        ...
        try:
            ...
            raise exceptions.TlsException("Cannot load trusted certificates ({}, {}).".format(ca_pemfile, ca_path))
...
def create_client_context(
    cert: str = None, sni: str = None, address: str = None, verify: int = SSL.VERIFY_NONE, **sslctx_kwargs
) -> SSL.Context:
    """
    Args:
    ...
    def verify_callback(conn: SSL.Connection, x509: SSL.X509, errno: int, depth: int, is_cert_verified: bool) -> bool:
        if is_cert_verified and depth == 0 and not sni:
            ...
            conn.cert_error = exceptions.InvalidCertificateException(
                "Certificate verification error for {}: {} (errno: {}, depth: {})".format(
                    sni, SSL._ffi.string(SSL._lib.X509_verify_cert_error_string(errno)).decode(), errno, depth
                )
            )
    ...
    if sni:
        ...
        SSL._lib.X509_VERIFY_PARAM_set_hostflags(
            param, SSL._lib.X509_CHECK_FLAG_NO_PARTIAL_WILDCARDS | SSL._lib.X509_CHECK_FLAG_NEVER_CHECK_SUBJECT
        )
        SSL._openssl_assert(SSL._lib.X509_VERIFY_PARAM_set1_host(param, sni.encode("idna"), 0) == 1)
