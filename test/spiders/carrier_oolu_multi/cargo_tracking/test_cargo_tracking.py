...
def test_cargo_tracking_handler(sub, mbl_no, sample_loader):
    ...
    option = CargoTrackingRule.build_request_option(search_no=mbl_no, task_id=1)
    ...
    results = list(CargoTrackingRule._handle_response(response=response, search_type=SHIPMENT_TYPE_MBL, task_id=1))
...
def test_cargo_tracking_handler_no_mbl_error(sub, mbl_no, expect_exception, sample_loader):
    ...
    with pytest.raises(expect_exception):
        list(CargoTrackingRule._handle_response(response=response, search_type=SHIPMENT_TYPE_MBL, task_id=1))
