...
def verify(results):
    ...
    assert isinstance(results[19], RoutingRequest)
    formdata = {
        'f_cmd': ContainerStatusRoutingRule.f_cmd,
        'cntr_no': 'UETU5848906',
        'bkg_no': 'SZPVF2740514',
    ...
    assert results[19].request.body == convert_formdata_to_bytes(formdata)

    assert isinstance(results[20], RoutingRequest)
    formdata = {
        'f_cmd': ReleaseStatusRoutingRule.f_cmd,
        'cntr_no': 'UETU5848906',
        'bkg_no': 'SZPVF2740514',
    }
    assert results[20].request.body == convert_formdata_to_bytes(formdata)

    assert isinstance(results[21], RoutingRequest)
    formdata = {
        'f_cmd': RailInfoRoutingRule.f_cmd,
        'cop_no': 'CSZP9916113073',
    }
    assert results[21].request.body == convert_formdata_to_bytes(formdata)
