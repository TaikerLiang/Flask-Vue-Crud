...
from test.spiders.terminal_tideworks_z985 import container_detail
from crawler.spiders.terminal_tideworks_z985 import TerminalOictSpider
...
@pytest.mark.parametrize(
    'sub,container_no,',
    [
        ('01_basic', 'MRSU3949609'),
    ],
)
def test_container_detail_routing_rule(sub, container_no, sample_loader):
    httptext = sample_loader.read_file(sub, 'sample.html')

    container_url = f'/fc-OICT/import/default.do?method=container&eqptNbr={container_no}&gkey=17642406'
    request_option = ContainerDetailRoutingRule.build_request_option(
        container_url=container_url,
        company_info=TerminalOictSpider.company_info
    )
