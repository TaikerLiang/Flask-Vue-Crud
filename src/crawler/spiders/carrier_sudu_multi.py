...
class CarrierSuduSpider(BaseMultiCarrierSpider):
    ...
    def start(self):
        for s_no, t_id in zip(self.search_nos, self.task_ids):
            option = PageInfoRoutingRule.build_request_option(mbl_no=s_no, task_id=t_id)
...
class MblSearchResultRoutingRule(BaseRoutingRule):
    ...
    @classmethod
    def build_request_option(cls, basic_request_spec: BasicRequestSpec, mbl_state: MblState, task_id: str) -> RequestOption:
    ...
    def handle(self, response):
        ...
        if mbl_state == MblState.FIRST:
            if self.is_multi_containers(response=response):
                ...
                yield MblSearchResultRoutingRule.build_request_option(
                    basic_request_spec=basic_request_spec, mbl_state=MblState.MULTIPLE, task_id=task_id,
                )
            else:
                yield ContainerDetailRoutingRule.build_request_option(
                    basic_request_spec=basic_request_spec, mbl_state=MblState.SINGLE, task_id=task_id,
                )
...
class ContainerDetailRoutingRule(BaseRoutingRule):
    ...
    def handle(self, response):
        ...
        if not voyage_spec:
            for result in self.__handle_detail_page(response=response, container_key=container_key, task_id=task_id):
                if isinstance(result, BaseCarrierItem):
                    yield result
            ...
            yield MblSearchResultRoutingRule.build_request_option(
                basic_request_spec=basic_request_spec, mbl_state=mbl_state, task_id=task_id,
            )
        else:
            yield VoyageRoutingRule.build_request_option(
                basic_request_spec=basic_request_spec,
    ...
    @classmethod
    def __handle_detail_page(cls, response, container_key: str, task_id: str):
        # parse
...
class VoyageRoutingRule(BaseRoutingRule):
    ...
    @classmethod
    def build_request_option(
        cls, basic_request_spec: BasicRequestSpec, voyage_spec: VoyageSpec, mbl_state: MblState, task_id: str,
    ) -> RequestOption:
        j_idt2 = 'j_idt8' if basic_request_spec.j_idt == 'j_idt6' else 'j_idt9'
        search_form = f'{basic_request_spec.j_idt}:searchForm'
        form_data = {
