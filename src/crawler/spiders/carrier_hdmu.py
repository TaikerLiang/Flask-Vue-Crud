...
class Cookies1RoutingRule(BaseRoutingRule):
    ...
    def handle(self, response):
        ...
        if cookies:
            yield Cookies2RoutingRule.build_request_option(search_no=search_no, cookies=cookies)
        else:
            yield ForceRestart()
    ...
    def handle(self, response):
        ...
        if self._search_type == SHIPMENT_TYPE_BOOKING and self._no_tracking_results(response=response):
            ...
            if not self._item_recorder.is_item_recorded(key=(VESSEL, search_no)):
                ...
                vessel_item = VesselItem(**vessel_info, vessel_key=vessel_info["vessel"])
...
class ContainerRoutingRule(BaseRoutingRule):
    ...
    def build_request_option(
        cls, search_no, search_type, container_index, h_num, cookies: Dict, under_line: bool = False
    ):
        url_plug_in = "/_" if under_line else ""
