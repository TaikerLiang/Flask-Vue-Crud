...
class MainRoutingRule(BaseRoutingRule):
    ...
    def _is_search_no_invalid(response: scrapy.Selector):
        error_messages = [
            "No matching tracking information. Please try again.",
            "Your reference number was not found, please review and try again",
        ]
        err_msg = response.css("div#ctl00_ctl00_plcMain_plcMain_pnlTrackingResults > h3::text").get()

        if err_msg in error_messages:
            return True
